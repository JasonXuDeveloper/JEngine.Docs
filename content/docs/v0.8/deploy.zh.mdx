...
<Callout type="warn" title="如果不打算使用远程模式（[热更资源](./ab.md)文章内有提到），可以忽略本文">
...
<Callout type="info" title="静态资源服务器可以用nginx/apache/tomcat部署，也可以用python的simple http server等其他的方式部署，">
把Bundles放到网页根目录下即可，

...
| 技能 | 命令 | 覆盖内容 |
|------|------|---------|
| **JAction** | `/jaction` | 链式操作、零分配模式、并行执行 |
| **JObjectPool** | `/jobjectpool` | 线程安全的对象池及正确的生命周期管理 |
| **游戏模式** | `/game-patterns` | 零GC游戏模式 — 生成器、冷却系统、战斗系统、持续伤害、生命回复 |

### JEngine.UI（运行时 + 编辑器）

| 技能 | 命令 | 覆盖内容 |
|------|------|---------|
| **MessageBox** | `/messagebox` | 带对象池的异步模态对话框 |
| **编辑器 UI** | `/editor-ui` | 使用 JEngine UI 组件构建编辑器工具（JButton、JStack、Tokens 等） |

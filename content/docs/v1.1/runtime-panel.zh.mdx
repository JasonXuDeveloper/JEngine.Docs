...
<Accordion title="Text Settings（文本设置）">
<Callout type="info">
v1.1.6 新增 — Bootstrap 现在在 Inspector 中暴露 `BootstrapText` 结构体，包含 33 个可配置的字符串字段，覆盖热更流程中所有面向用户的文本。
</Callout>

覆盖任意字段即可本地化或自定义文本。默认值为英文。字段在 Inspector 中按 6 个标签页组织：
...
**使用场景：** 将所有热更文本本地化为任意语言，或自定义措辞以匹配你的游戏。
</Accordion>
</Accordions>

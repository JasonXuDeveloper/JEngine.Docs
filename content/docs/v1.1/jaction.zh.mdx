...
| 方法 | 返回值 | 描述 |
|------|--------|------|
| `.Execute(float timeout = 0)` | `JActionExecution` | 同步执行（阻塞主线程直到完成） |
| `.ExecuteAsync(float timeout = 0)` | `JActionExecutionHandle` | 通过 PlayerLoop 异步执行（推荐）。可 await——返回 `JActionExecution`。 |
...
| 方法 | 描述 |
|------|------|
| `Cancel()` | 取消此 JAction 的**所有**活跃执行。并行模式下，使用 `handle.Cancel()` 取消特定执行。 |
...
v1.1 新增 `JAction.Parallel()`，支持同一 Action 的并发执行，提供逐次取消和任务快照隔离。

### 新增类型

| 类型 | 说明 |
|------|------|
| `JActionExecution` | 已完成的并行执行结果（readonly struct） |
| `JActionExecutionHandle` | 运行中的并行执行句柄 — 可取消或等待 |
| `JActionExecutionContext` | 内部逐次执行状态，用于并发隔离 |
...
在 `ExecuteAsync()` 之后对 JAction 的修改不会影响已运行的执行。每次调用都会捕获当前任务列表的快照。
...
| `.Executing` | 操作是否仍在运行 |
| `.Cancel()` | 仅取消此次特定执行 |
| `.AsUniTask()` | 转换为 `UniTask<JActionExecution>` |
| `GetAwaiter()` | 可直接 await |

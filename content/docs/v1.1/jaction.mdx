---
title: JAction
description: Chainable action framework with zero-allocation async execution
icon: Zap
---
...
<Callout type="info" title="Preemptive Execution">
...
|--------|---------|-------------|
| `.Execute(float timeout = 0)` | `JActionExecution` | Execute synchronously (blocks main thread until complete) |
| `.ExecuteAsync(float timeout = 0)` | `JActionExecutionHandle` | Execute asynchronously via PlayerLoop (recommended). Awaitable — `await` returns `JActionExecution`. |

<Callout type="error" title="Prefer ExecuteAsync() for Better Performance">
...
| Method | Description |
|--------|-------------|
| `Cancel()` | Cancel **all** active executions of this JAction. In parallel mode, use `handle.Cancel()` to cancel a specific execution. |
...
<Tabs items={['Loading Sequence', 'Retry Logic', 'Animation Sequence']}>
<Tab value="Loading Sequence">
```csharp
...
## Parallel Execution (New in v1.1)

v1.1 adds `JAction.Parallel()` for concurrent execution of the same action with per-execution cancellation and task snapshot isolation.

### New Types

| Type | Description |
|------|-------------|
| `JActionExecution` | Result of a completed parallel execution (readonly struct) |
| `JActionExecutionHandle` | Handle to a running parallel execution — cancel or await it |
| `JActionExecutionContext` | Internal per-execution state for concurrent isolation |
...
// Await both — per-execution cancellation state
...
Modifications to the JAction after `ExecuteAsync()` don't affect already-running executions. Each call captures a snapshot of the current task list.

### JActionExecutionHandle API

| Member | Description |
|--------|-------------|
| `.Action` | The underlying JAction |
| `.Cancelled` | Whether this execution was cancelled |
| `.Executing` | Whether the action is still running |
| `.Cancel()` | Cancel this specific execution only |
| `.AsUniTask()` | Convert to `UniTask<JActionExecution>` |
| `GetAwaiter()` | Directly awaitable |

## Best Practices

<Accordions>
<Accordion title="Prefer ExecuteAsync() over Execute()">
Synchronous execution blocks the main thread by spinning, causing frame drops and UI freezes. Only use `Execute()` for unit tests or editor scripts.
</Accordion>

<Accordion title="Use static lambdas with state parameters">
For performance-critical code, use static lambdas with state parameters to avoid GC allocation from closures.
</Accordion>

<Accordion title="Keep .Do() callbacks lightweight">
Code inside callbacks runs atomically and cannot be preempted. Long-running or blocking operations will freeze the game.
</Accordion>

<Accordion title="Use the using pattern">
Automatically dispose and return JAction instances to the pool: `using var action = await ...ExecuteAsync()`
</Accordion>

<Accordion title="Set appropriate timeouts">
Prevent infinite waits in production by setting reasonable timeout values.
</Accordion>

<Accordion title="State overloads only work with reference types">
For value types (int, float, struct), use closures or wrap in a class instead.
</Accordion>
</Accordions>

---
title: 迁移指南
description: 从 JEngine 旧版本迁移到 v1.1 的指南
icon: ArrowRightLeft
---

本指南帮助您从 JEngine 旧版本迁移到 v1.1。
...
<Step>
### 更新 Core 包
删除 `Packages/com.jasonxudeveloper.jengine.core`，通过 UPM 重新安装：
```bash
openupm add com.jasonxudeveloper.jengine.core
```
手动安装请访问 [OpenUPM 页面](https://openupm.com/packages/com.jasonxudeveloper.jengine.core/)。
</Step>
...
```
手动安装请访问 [OpenUPM 页面](https://openupm.com/packages/com.jasonxudeveloper.jengine.ui/)。
</Step>

<Step>
### 更新 MessageBox 命名空间
将所有使用 MessageBox 的地方的 `using JEngine.Core.Misc;` 替换为 `using JEngine.UI;`。
...
<Step>
### 添加 PromptInitializer
创建 `Assets/Scripts/PromptInitializer.cs`，将 MessageBox 连接到新的 Prompt 抽象层：
```csharp
using JEngine.Core;
using JEngine.UI;
...
```
手动安装请访问 [OpenUPM 页面](https://openupm.com/packages/com.jasonxudeveloper.jengine.util/)。
</Step>
</Steps>
...
| 变更 | v1.0 | v1.1 |
|------|------|------|
| MessageBox 命名空间 | `JEngine.Core.Misc` | `JEngine.UI` |
| MessageBox 所属包 | `jengine.core` | `jengine.ui`（需单独安装） |
| Bootstrap 对话框 | 直接调用 `MessageBox.Show()` | `Prompt.ShowDialogAsync()`（委托） |

## 从 v1.1.x 升级

直接通过 UPM 更新包即可——v1.1.x 补丁版本之间无破坏性变更。
...
<Cards>
  <Card title="常见问题" href="/zh/docs/v1.1/faq" description="常见问题和解决方案" />

...
description: Manage hot update packages with independent main and sub-package support
icon: Package
---
...
<Files>
  <Folder name="Assets/HotUpdate" defaultOpen>
    <Folder name="Main (main package resources)" defaultOpen>
      <Folder name="Prefabs" />
      <Folder name="Scenes" />
      <Folder name="UI" />
    </Folder>
    <Folder name="AddOn1 (sub-package 1)" />
    <Folder name="Code (auto-included in main)" />
  </Folder>
</Files>
...
<Files>
  <Folder name="Bundles" defaultOpen>
    <Folder name="WebGL (platform)" defaultOpen>
      <Folder name="main (package)" defaultOpen>
        <Folder name="24120115300 (version 1)" />
        <Folder name="24120115400 (version 2)" />
      </Folder>
    </Folder>
  </Folder>
</Files>
...
All package management methods are static methods in the `Bootstrap` class.

### CreateOrGetPackage

Create or get a resource package instance for managing hot update sub-packages.
...
Check and update the specified resource package, including version checking, resource downloading, and initialization.
...
Asynchronously load hot update scenes from the specified resource package.
...
Delete local cache files for the specified resource package.
...
<Tabs items={['Editor', 'Android', 'iOS', 'Windows']}>
<Tab value="Editor">`UnityProject/yoo/`</Tab>
...
<Tab value="XOR">`EncryptionOption.Xor` — Lightweight, fast. Recommended for most cases.</Tab>
<Tab value="AES">`EncryptionOption.Aes` — Balanced security and performance.</Tab>
<Tab value="ChaCha20">`EncryptionOption.ChaCha20` — Highest security.</Tab>
</Tabs>
...
<Accordions>
<Accordion title="Package Division Principles">
...
- Keep package sizes reasonable (recommend under 50MB per package)

...
<Callout type="info" title="如果只需要使用本框架，本文可以略过；">
如果对本框架原理感兴趣，建议阅读本文
</Callout>
...
![flowchart](/images/principle-flowchart.png)
...
<Callout type="warn" title="AssetBundle的平台若于运行平台不对应，可能会造成Shader丢失（如编辑器下运行安卓平台的AssetBundle，或安卓平台运行编辑器平台的AssetBundle）">
...
<Callout type="warn" title="非开发模式时请确保出包（例如打APK时），BuildSetting内不包含热更场景，不然会导致被打入游戏主包的场景无法热更">
...
如果正好触发了Time.fixedTimestep的频率，第3帧会调用B的FixedUpdate，不会调用A的任何事件（与上面提到的同理，A默认注册了空的FixedUpdate用于占位）。第3帧的时候会调用A和B的Update，因为他俩都注册了这个事件。第3帧的时候会调用B的LateUpdate，不会调用A的任何事件（理由如上）。Update和LateUpdate会一直循环下去，而FixedUpdate只会在Time.fixedTimestep的固定频率下被调用。
</Callout>

<Callout type="warn" title="未触发 FixedUpdate 的场景">
如果没有触发Time.fixedTimestep的频率，第3帧会调用A和B的Update，因为他俩都注册了这个事件。第3帧的时候会调用B的LateUpdate，不会调用A的任何事件（理由如上）。Update和LateUpdate会一直循环下去，而FixedUpdate只会在Time.fixedTimestep的固定频率下被调用。
</Callout>

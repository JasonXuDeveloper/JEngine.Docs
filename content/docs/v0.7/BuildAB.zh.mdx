...
<Callout type="info" title="合理的粒度管理可以减少生成出来的文件的数量">
...
<Callout type="info" title="只有编辑器下才需要配置，该操作不影响真机">
只有创建了新分包才需要对打包进行配置
</Callout>
...
<Callout type="info" title="需要在```Assets/Dependencies/BundleMaster/Editor/BundleMasterEditor/BuildSettings```内找到```AssetLoadSetting```（普通分包配置文件）进行配置">

...
| 模式 | 说明 | 使用场景 |
|------|------|----------|
| **Editor Dev Mode（红色）** | 直接使用本地热更资源 | 开发调试 |
| **Host Play Mode（绿色）** | 从服务器下载热更资源 | 测试和发布 |
</Accordion>
...
| 字段 | 说明 |
|------|------|
| **Default Host Server** | 主要下载地址（如 `https://cdn.domain.com`），不要包含平台/分包路径 |
| **Enable Fallback Host Server** | 启用备用服务器开关 |
| **Fallback Host Server** | 备用服务器地址，主服务器失败时自动切换 |
...
**其他字段：**
- **Package Name** - 主热更包名称（默认：`main`）
- **Hot Update Assemblies** - 热更程序集列表
- **Hot Update Scenes** - 场景配置（与 YooAsset 关联）
- **Hot Update Classes** - 入口类（默认：`EntryPoint`）
- **Hot Update Methods** - 入口方法（必须是 `static`，可返回 `void`/`ValueTask`/`Task`/`UniTask`）
...
| 字段 | 说明 |
|------|------|
| **Dynamic Secret Key** | 解密密钥文件（通常在 `Assets/HotUpdate/Obfuz/`） |
| **Encryption Option** | 必须与编辑器面板打包时一致（Xor/Aes/ChaCha20） |
...
<Step>配置 Default Host Server（必需）</Step>
<Step>设置正确的 Encryption Option（与打包时一致）</Step>
<Step>选择对应的 Target Platform</Step>
<Step>运行游戏</Step>
...
<Steps>
<Step>设置 Editor Mode 为 **Host Play Mode**</Step>
<Step>设置 Target Platform 为 **Standalone**</Step>
...
<Steps>
<Step>勾选 "Enable Fallback Host Server"</Step>
<Step>在显示的 "Fallback Host Server" 字段中输入备用服务器地址</Step>

...
<Callout type="info">
安装后，使用 `using JEngine.Util;` 来访问 JAction 和 JObjectPool 类。详细用法请参见 [JAction 文档](./jaction) 和 [JObjectPool 文档](./jobjectpool)。
</Callout>
...
2. 在 `Inspector` 的 `Development Settings` 区域内确保 `Editor Mode` 为 `Editor Dev Mode`（红色按钮）
</Step>
...
<Step>
### 配置打包设置
- `Package Name` 选择需要打包的分包（通常是 `main`）
- `Build Target` 点 `Set to Current Active Target`
</Step>
...
<Step>
### 构建热更资源
- **代码或 Obfuz 配置有变化**：点 `Build All Hot Update Res (Code + Assets)`
- **仅资源变化**：点 `Build Hot Update Assets Only`

<Callout type="warn">
如果出现错误，请先查阅 [常见问题 - 打包和发布](/zh/docs/v1.0/faq#打包和发布) 章节。也可以参考 [YooAsset 文档](https://www.yooasset.com/docs/FAQ)、[HybridCLR 文档](https://www.hybridclr.cn/docs/help/commonerrors) 和 [Obfuz 文档](https://www.obfuz.com/docs/help/faq)。
</Callout>
</Step>
...
4. `Server Settings` 中输入 CDN 地址（如 `https://cdn.domain.com`）
5. `Security Settings` 中选择打包时使用的加密模式
6. `Asset Settings` 中选择目标平台
</Step>

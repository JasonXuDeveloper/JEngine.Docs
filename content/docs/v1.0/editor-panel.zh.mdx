...
**Build Target** - 点击 `Set to Current Active Target` 设置当前平台（Android、iOS、Windows、MacOS、WebGL 等）
</Accordion>
...
<Accordion title="Hot Update Scenes（热更新场景）">
...
<Step>
### 设置平台
点击 `Set to Current Active Target` 设置构建平台
</Step>
...
### 构建资源
根据修改内容选择对应的构建按钮：
- **代码有变化**：点击 `Build All Hot Update Res (Code + Assets)`
- **仅资源变化**：点击 `Build Hot Update Assets Only`
</Step>

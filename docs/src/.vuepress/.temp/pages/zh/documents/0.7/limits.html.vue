...
<p>开发时有额外GC，但是Release出热更DLL，真机下就没了</p>
</li>
<li>
<p>多线程调用同函数需要预热优化性能（非必须）</p>
<p>参考ILRuntime文档</p>
</li>
</ul>
...
<li>
<p><s>LitJson反序列化限制</s>（已经在JEnginev0.6.2修复并支持了）</p>
...
<span class="token punctuation">{</span>
  ...
  <span class="token punctuation">{</span>
      ...
      lst<span class="token punctuation">.</span><span class="token function">Add</span><span class="token punctuation">(</span><span class="token keyword">new</span> <span class="token constructor-invocation class-name">MonoClass</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span><span class="token comment">//出错，因为Mono跨域继承了，无法强转为热更类型</span>
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>

...
::: danger 推荐使用 ExecuteAsync() 以获得更好的性能
...
1. **始终优先使用 `ExecuteAsync()` 而非 `Execute()`** - 同步执行通过自旋阻塞主线程，导致掉帧和 UI 冻结。仅在单元测试或编辑器脚本中使用 `Execute()`。

2. **在性能关键代码中使用 `static` lambda 配合状态参数**，以避免 GC 分配

3. **保持 `.Do()` 回调轻量** - 回调内的代码以原子方式运行，无法被抢占。长时间运行或阻塞的操作会冻结游戏。

4. **使用 `using` 模式**，自动释放并将 JAction 实例返回池中（例如 `using var action = await ...ExecuteAsync()`）

5. **设置适当的超时时间**，防止生产环境中的无限等待

6. **记住：状态重载仅适用于引用类型** - 对于值类型，请使用闭包或封装到类中
